...
type RealtimeGraph struct {
	BlankCodec
	Req  *RealtimeGraphReq
	Resp *RealtimeGraphResp
}

type RealtimeGraphReq struct {
	Market models.MarketType
	Code   [6]byte
	R1     [4]uint8
}
type RealtimeGraphResp struct {
	Count uint16
	List  []QuoteFrame
}
...
func (RealtimeGraph) FillReqHeader(ctx context.Context, header *ReqHeader) error {
	header.Type0 = 0x0100
	header.Method = 0x0537
	return nil
}

func (obj *RealtimeGraph) MarshalReqBody(ctx context.Context) ([]byte, error) {
...
func (obj *RealtimeGraph) UnmarshalResp(ctx context.Context, data []byte) error {
	var err error
	obj.Resp = &RealtimeGraphResp{}
	cursor := 0
